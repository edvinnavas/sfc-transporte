    ...
    public String ObtenerUbicaciones(String database, Integer dias_tracking) {
        ...
        try {
            ...
            sql = "SELECT DISTINCT "
                    ...
                    + "(V.ID_PAIS, V.ID_COMPANIA, V.ID_PLANTA, V.NUMERO_VIAJE, V.TIPO_ORDEN_VENTA, V.NUMERO_ORDEN_VENTA, STR_TO_DATE(SOD.DATETIME_UBICACION, '%Y-%m-%d %H:%i:%s'), SOD.IMEI, SOD.LATITUDE, SOD.LONGITUDE) NOT IN (SELECT F.ID_PAIS, F.ID_COMPANIA, F.ID_PLANTA, F.NUMERO_VIAJE, F.TIPO_ORDEN_VENTA, F.NUMERO_ORDEN_VENTA, F.FECHA_HORA, F.IMEI, F.LATITUDE, F.LONGITUDE FROM VIAJE_UBICACIONES F)";
            stmt = conn.createStatement();
