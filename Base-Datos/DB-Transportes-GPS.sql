...
SELECT B.CODIGO, A.NUMERO_VIAJE, A.FECHA_VIAJE
FROM VIAJES A LEFT JOIN PLANTA B ON (A.ID_PLANTA=B.ID_PLANTA) 
WHERE A.ESTADO='ACT' 
ORDER BY A.FECHA_VIAJE;


SELECT P.* FROM PAIS P WHERE P.ID_PAIS=6;
SELECT T.* FROM TRANSPORTISTA T WHERE T.ID_PAIS=6;

SELECT P.NOMBRE, T.NOMBRE, V.FECHA_VIAJE, COUNT(*) NO_VIAJES
FROM VIAJES V LEFT JOIN PLANTA P ON (V.ID_PLANTA=P.ID_PLANTA) LEFT JOIN TRANSPORTISTA T ON (V.ID_TRANSPORTISTA=T.ID_TRANSPORTISTA)
WHERE V.ID_PAIS=6 AND V.FECHA_VIAJE > '2023-07-01' AND V.TIPO_FLETE_VIAJE NOT LIKE '%FOB%'
GROUP BY P.NOMBRE, T.NOMBRE, V.FECHA_VIAJE
ORDER BY P.NOMBRE, T.NOMBRE, V.FECHA_VIAJE;

SELECT IFNULL(MAX(A.ID_SMS_OPEN),0)+1 MAX_ID FROM SMS_OPEN A;