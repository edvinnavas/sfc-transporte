    ...
    public Respuesta_WS_Viaje obtener_viajes(String fecha) {
        ...
        try {
            ...
            String cadenasql = "SELECT DISTINCT "
                    ...
                    + "(SELECT DISTINCT NVL(TRIM(SO.SDLOB),'-') FROM (SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN, B.SDLOB FROM PRODDTA.F4211 B UNION ALL SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN, B.SDLOB FROM PRODDTA.F42119 B) SO WHERE SO.SDDCTO=C.TODCTO AND SO.SDDOCO=C.TODOCO AND ROWNUM=1) TIPO_FLETE_VIAJE "
                    + "FROM "
            ...
            while (rs.next()) {
                ...
                String TIPO_FLETE_VIAJE;
                if (rs.getString(21).equals("CIF")) {
                    TIPO_FLETE_VIAJE = "CIF";
                } else {
                    TIPO_FLETE_VIAJE = "FOB";
                }
                viaje.setTIPO_FLETE_VIAJE(TIPO_FLETE_VIAJE);
