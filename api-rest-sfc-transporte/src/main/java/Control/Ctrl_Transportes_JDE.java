    ...
    public Respuesta_WS_Viaje obtener_viajes(String fecha) {
        ...
        try {
            ...
            String cadenasql = "SELECT DISTINCT "
                    + "(SELECT TRIM(B.ALCTR) FROM PRODDTA.F0116 B WHERE ROWNUM=1 AND B.ALAN8=C.TDKCOO) CODIGO_PAIS, "
                    + "(SELECT TRIM(B.DRDL01) FROM PRODCTL.F0005 B WHERE TRIM(B.DRSY)='00' AND TRIM(B.DRRT)='CN' AND TRIM(B.DRKY)=(SELECT TRIM(B.ALCTR) FROM PRODDTA.F0116 B WHERE ROWNUM=1 AND B.ALAN8=C.TDKCOO)) NOMBRE_PAIS, "
                    + "C.TDKCOO CODIGO_COMPANIA, "
                    + "(SELECT TRIM(B.ABALPH) FROM PRODDTA.F0101 B WHERE B.ABAN8=C.TDKCOO) NOMBRE_COMPANIA, "
                    ...
                    + "NVL(A.TMCARS,0) CODIGO_TRANSPORTISTA, "
                    + "NVL((SELECT TRIM(B.ABALPH) FROM PRODDTA.F0101 B WHERE B.ABAN8=A.TMCARS),'-') NOMBRE_TRANSPORTISTA, "
                    + "C.TDDCTO TIPO_ORDEN_VENTA, "
                    + "C.TDDOCO NUMERO_ORDEN_VENTA, "
                    + "NVL((SELECT DISTINCT SO.SDAN8 FROM (SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDAN8 FROM PRODDTA.F4211 B UNION ALL SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDAN8 FROM PRODDTA.F42119 B) SO WHERE SO.SDDCTO=C.TDDCTO AND SO.SDDOCO=C.TDDOCO AND ROWNUM=1),0) CODIGO_CLIENTE, "
                    + "NVL((SELECT TRIM(B.ABALPH) FROM PRODDTA.F0101 B WHERE B.ABAN8=(SELECT DISTINCT SO.SDAN8 FROM (SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDAN8 FROM PRODDTA.F4211 B UNION ALL SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDAN8 FROM PRODDTA.F42119 B) SO WHERE SO.SDDCTO=C.TDDCTO AND SO.SDDOCO=C.TDDOCO AND ROWNUM=1)),'-') NOMBRE_CLIENTE, "
                    + "NVL((SELECT DISTINCT SO.SDSHAN FROM (SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN FROM PRODDTA.F4211 B UNION ALL SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN FROM PRODDTA.F42119 B) SO WHERE SO.SDDCTO=C.TDDCTO AND SO.SDDOCO=C.TDDOCO AND ROWNUM=1),0) CODIGO_CLIENTE_DESTINO, "
                    + "NVL((SELECT TRIM(B.ABALPH) FROM PRODDTA.F0101 B WHERE B.ABAN8=(SELECT DISTINCT SO.SDSHAN FROM (SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN FROM PRODDTA.F4211 B UNION ALL SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN FROM PRODDTA.F42119 B) SO WHERE SO.SDDCTO=C.TDDCTO AND SO.SDDOCO=C.TDDOCO AND ROWNUM=1) AND ROWNUM=1),'-') NOMBRE_CLIENTE_DESTINO, "
                    + "NVL((SELECT DISTINCT NVL(TRIM(SO.SDZON),'-') FROM (SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN, B.SDZON FROM PRODDTA.F4211 B UNION ALL SELECT B.SDKCOO, B.SDDCTO, B.SDDOCO, B.SDSHAN, B.SDZON FROM PRODDTA.F42119 B) SO WHERE SO.SDDCTO=C.TDDCTO AND SO.SDDOCO=C.TDDOCO AND ROWNUM=1),'-') TIPO_FLETE_VIAJE "
                    + "FROM "
                    + "PRODDTA.F4960 A LEFT JOIN PRODDTA.F49621 C ON (A.TMVMCU=C.TDVMCU AND A.TMLDNM=C.TDLDNM) "
                    + "WHERE "
                    + "A.TMLOAD=" + TMLOAD + " AND C.TDDOCO>0 AND TRIM(C.TDKCOO) IS NOT NULL";
